...
def test_init_process_is_protected():
    classification, reason = classify_signal(1, signal.SIGKILL, own_pid=4242)
    assert classification is OperationClass.REFUSED
    assert "halts the host" in reason


def test_kernel_pid_zero_is_protected():
    assert classify_signal(0, signal.SIGTERM, own_pid=4242)[0] is OperationClass.REFUSED


def test_agent_cannot_signal_itself():
    classification, reason = classify_signal(4242, signal.SIGTERM, own_pid=4242)
    assert classification is OperationClass.REFUSED
...
def test_process_group_signals_are_refused():
    ...
    assert classify_signal(-1, signal.SIGTERM, own_pid=4242)[0] is OperationClass.REFUSED


@pytest.mark.parametrize(
    "sig", [signal.SIGTERM, signal.SIGHUP, signal.SIGKILL, signal.SIGUSR1, signal.SIGCONT]
)
...
async def test_signal_to_self_refused_even_when_fully_enabled():
    policy = SystemPolicy(enabled=True, allow_mutations=True)
    with pytest.raises(UnsafeOperationError):
        await send_signal(policy, os.getpid(), signal.SIGTERM)


async def test_signal_to_init_refused_even_when_fully_enabled():
    policy = SystemPolicy(enabled=True, allow_mutations=True)
    with pytest.raises(UnsafeOperationError):
        await send_signal(policy, 1, signal.SIGKILL)
...
async def test_signalling_a_dead_process_reports_cleanly(tmp_path):
    ...
    result = await send_signal(policy, 4_000_000, signal.SIGTERM)
