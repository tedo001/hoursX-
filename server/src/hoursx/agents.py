...
from hoursx.providers.types import ChatMessage, ChatRequest, ChatRole, ToolCall
...
class AgentRuntime:
    ...
    async def execute_run(self, run_id: str, *, _depth: int = 0) -> None:
        ...
        snap = await self._claim(run_id, from_status=("queued", "running"))
        if snap is None:
            return
    ...
    async def _claim(
        ...
        async with self._services.db.session() as db:
            run = await db.get(Run, run_id)
            if run is None or run.status not in from_status:
                return None
