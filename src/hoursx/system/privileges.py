...
_SIGNALS_ALLOWED = {
    ...
    signal_module.SIGHUP,
    signal_module.SIGUSR1,
    signal_module.SIGUSR2,
...
def classify_signal(pid: int, signal_number: int, *, own_pid: int) -> tuple[OperationClass, str]:
    ...
    try:
        resolved = signal_module.Signals(signal_number)
    except ValueError:
        return OperationClass.REFUSED, f"unknown signal number {signal_number}"
    if resolved not in _SIGNALS_ALLOWED:
        return OperationClass.REFUSED, f"{resolved.name} is not in the permitted signal set"
    return OperationClass.MUTATE, f"sending {resolved.name} to pid {pid} requires approval"
